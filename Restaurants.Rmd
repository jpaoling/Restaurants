...
Next, We will construct a table containing the difference between inspections and the total score for a given inspection. Also, we also have to make sure that the distributions of the target in training and test data are similar.   

```{r}
make_diff_df <- function(x) {
  x %>% 
...
grid.arrange(train_df %>% make_diff_df() %>% plot_diff(),
             test_df %>% make_diff_df() %>%  plot_diff())
```

The distribution of the target seems reasonably similar across training and test data. So now, we remove the original data set and the test data from the workspace and save the latter to a file
...
train_diff_score <- train_df %>% make_diff_df()
train_diff_score
```
...
train_df_complete <- train_diff_score %>% 
  left_join(train_inspec_viol, by = c("id", "inspection_date")) %>% 
  filter(!is.na(difference)) 
train_df_complete
```
