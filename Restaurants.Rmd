...
For later use, we construct dummy variables for the different violation groups, with 'crit_other' as base group:

```{r}
for(level in unique(restaurants_raw$violation_group)){
  if (is.na(level) | level == "crit_other") {
    next
  } else {
...
train_df_feat_1 <- train_df %>% 
  select(id, inspection_date, dummy_InitialInspection, score, 
         matches("grade_"), -grade_date, matches("viol_")) %>% 
  group_by(id, inspection_date) %>% 
  arrange(id, inspection_date) %>% 
  summarise_at(vars(starts_with("grade"), c("dummy_InitialInspection", "score")), 
               .funs = list(first)) %>% 
  ...
  left_join(
    train_df %>% 
      ...
      group_by(id, inspection_date) %>% 
      arrange(id, inspection_date) %>% 
      summarise_at(vars(starts_with("viol")), ~ sum(.)) %>% 
      ungroup(),
    by = c("id", "inspection_date")
  ) %>% 
  filter(!is.na(days_until_next)) %>% 
  select(days_until_next, score, everything(), -id, -inspection_date)
```

Let us check the number of NA's for each variable and the number of NA rows overall:

```{r}
train_df_feat_1 %>% 
  summarise_all(funs(round(sum(is.na(.))/n(), 3)))

train_df_feat_1 %>% 
  summarise(n_rows_na = round(sum(is.na(grade_A) | is.na(viol_not_scored))/n(), 3))
```
...
```{r}
(m1 <- rpart(days_until_next ~ ., data = train_df_feat_1, method = "anova"))
```
