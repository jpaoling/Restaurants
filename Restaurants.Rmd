...
Let us look at the distribution of violation_code again, now using the information which group a given violation belongs to.
...
The proportion of grades pending and not yet graded is higher for earlier inspections, which is not surprising. Next, we'll look at how the number of NAs for 'grade' relates to the number of NAs for 'inspection_type.' As a reminder, above we saw that there are 187702 missing values for 'grade' overall.
...
Indeed, the vast majority of observations that belong to the 'not_scored' violation group has a missing score value (97 %). 

```{r}
round(16086/19514, 2)
```

These observations account for 82 % of all NA's for score.
