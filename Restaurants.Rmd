...
Let us construct dummy variables for the categories above which will come in handy, once we get to the task of prediction, we'll use 'LimitedInspection' as base category:

```{r}
for(level in unique(restaurants_raw$inspection_type2)){
  
  if (level == "LimitedInspection") {
    next
  } else {
    restaurants_raw[paste("dummy", level, sep = "_")] <- 
      ifelse(restaurants_raw$inspection_type2 == level &
             restaurants_raw$inspection_type2 != "SecondComplianceInspection", 
             1, 0)
  }
}
...
Let us, for later purposes, create a dummy variable for the different levels of 'grade', where we choose 'NA' to be our base level:
...
For later use, we construct dummy variables for the different violation groups, with 'NA' as base group:
...
So now, we remove the original data set and the test data from the workspace and save the latter to a file:
...
Next, We will construct a table containing the difference between inspections and the total score for a given inspection. Also, we have to make sure that the distributions of the target in training and test data are similar.   

```{r}
make_diff_score_df <- function(x) {
  x %>% 
...
From now on we will be working with training data exclusively.

```{r}
# Data frame containing restaurant id, inspection date, total score and time between inspections
train_diff_score <- train_df %>% make_diff_score_df()
train_diff_score
```

Next, we construct a data frame containing id, inspection date and the dummy variables for inspection type, violation group and grade: 
...
(train_inspec_grade_viol <- train_df %>% 
  select(id, inspection_date, starts_with("dummy")) %>% 
  arrange(id, inspection_date))
```

Now, we will transform the data frame 'train_inspec_grade_viol' so that each row corresponds to an inspection on a given date and at a given restaurant: 

```{r}
# Tranformation of sample_violations:
(train_inspec_grade_viol <- train_inspec_grade_viol %>% 
  group_by(id, inspection_date) %>% 
...
We'll join the table 'train_inspec_grade_viol' with the table 'train_diff_score', created earlier, for convenience reproduced below:
...
(train_df_complete <- train_diff_score %>% 
  inner_join(train_inspec_grade_viol, by = c("id", "inspection_date")) %>% 
  filter(!is.na(difference))) 
```
...
In the first model we include the total score on a given inspection, and all dummy variables for inspection type, grade and violation groups, respectively.

```{r}
(m1 <- rpart(difference ~ ., data = train_df_complete, method = "anova"))
```
